...
func HandleResourcePoolAndServerPlanning(db *gorm.DB, planId int64, cloudProductPlanningList []*entity.CloudProductPlanning, currentUserId string) error {
	...
	if _, err = capacity_planning.HandleBmsGWAndMasterServerNum(list, nodeRoleCodeBaselineMap, true, productIdList); err != nil {
		return err
	}
	...
	if err = db.Table(entity.ServerCapPlanningTable).Where("plan_id = ? and resource_pool_id not in (?)", planId, append(resourceIdList, 0)).Delete(&entity.ServerCapPlanning{}).Error; err != nil {
