...
import (
	...
	"code.cestc.cn/ccos/cnm/ops-base/tools/color"
)

const (
	OtherLevelAccess  = "ACCESS"
	OtherLevelCrash   = "CRASH"
	OtherLevelSql     = "SQL"
	OtherLevelGen     = "GEN"
	OtherLevelRequest = "REQUEST"
)
...
var (
	...
	_levelToLowercaseColorString = map[string]string{
		OtherLevelAccess:  color.Blue.Add(OtherLevelAccess),
		OtherLevelCrash:   color.Red.Add(OtherLevelCrash),
		OtherLevelSql:     color.Yellow.Add(OtherLevelSql),
		OtherLevelRequest: color.Green.Add(OtherLevelRequest),
		OtherLevelGen:     color.Cyan.Add(OtherLevelGen),
	}

	//_levelToLowercaseColorString = map[string]string{
	//	OtherLevelAccess:  OtherLevelAccess,
	//	OtherLevelCrash:   OtherLevelCrash,
	//	OtherLevelSql:     OtherLevelSql,
	//	OtherLevelRequest: OtherLevelRequest,
	//	OtherLevelGen:     OtherLevelGen,
	//}
)
...
func NewTimeEncoder() func(time.Time, zapcore.PrimitiveArrayEncoder) {
...
func NewConsoleEncoderWithOtherLevel(cfg *zapcore.EncoderConfig, otherLevel string) zapcore.Encoder {
	e := newConsoleEncoder(cfg, false)
	e.setOtherLevel(otherLevel)
	return e
}
...
func (c *consoleEncoder) setOtherLevel(level string) {
	c.otherLevel = level
}
...
func (c *consoleEncoder) EncodeEntry(ent zapcore.Entry, fields []zapcore.Field) (*buffer.Buffer, error) {
	...
	if c.LevelKey != "" && c.EncodeLevel != nil {
		...
		c.addTabIfNecessary(line)
		line.AppendString(_levelToLowercaseColorString[c.otherLevel])
	}

	// hostname
	c.addTabIfNecessary(line)
	line.AppendString(_config.ServiceName)

	// service name
	c.addTabIfNecessary(line)
	hostName, _ := os.Hostname()
	line.AppendString(hostName)

	putLineEncoder(lineEnc)

	c.addTabIfNecessary(line)
	line.AppendString("SYSTEM_LOG") // 兼容系统日志

	c.addTabIfNecessary(line)
	// FIXME 兼容系统日志，暂时先和日志内容放在一块
	if ent.Caller.Defined && len(c.CallerKey) > 0 && c.EncodeCaller != nil {
		line.AppendString(color.Magenta.Add(ent.Caller.String()))
